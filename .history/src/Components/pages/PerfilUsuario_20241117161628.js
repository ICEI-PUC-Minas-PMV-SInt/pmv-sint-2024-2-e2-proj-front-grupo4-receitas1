...
const PerfilUsuario = () => {
	...
	useEffect(() => {
		...
		const receitasSalvas = [...receitasEstaticas, ...userRecipes].filter(
			receita => savedRecipesIds.includes(receita.id)
		);
							...
							{receitasFiltradas.length > 0 ? (
								receitasFiltradas.map(receita => (
									<ReceitaCard
										...
										fotoUsuarioPostador={
											receita.fotoUsuarioPostador || defaultAvatar
										}
										usuarioPostador={receita.usuarioPostador || 'Desconhecido'}
									/>
								))
							) : (
