...
const AdicionarReceita = () => {
    ...
    const [filtrosSelecionados, setFiltrosSelecionados] = useState([]); // Certifique-se de que isso está aqui!
    ...
    useEffect(() => {
        ...
        if (receitasSalvas) {
            ...
            const receitasAdicionadas = JSON.parse(localStorage.getItem('userRecipes')) || [];
            ...
            sessionStorage.setItem('receitasRecomendadas', JSON.stringify(receitasAleatorias));
        }
    }, [user, navigate]);
    ...
    const handleSalvarReceita = () => {
        if (
            ...
            !foto ||
            !filtrosSelecionados.some(f =>
                ['Café da Manhã', 'Almoço', 'Jantar', 'Sobremesa', 'Lanche'].includes(f)
            )
        ) {
            alert('Por favor, preencha todos os campos e selecione uma categoria antes de salvar.');
            return;
        }
        ...
        const receitasExistentes = JSON.parse(localStorage.getItem('userRecipes')) || [];
        localStorage.setItem('userRecipes', JSON.stringify([...receitasExistentes, novaReceita]));
