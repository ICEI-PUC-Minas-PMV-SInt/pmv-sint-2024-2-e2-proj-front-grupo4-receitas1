...
const PaginaReceita = () => {
	...
	const [likes, setLikes] = useState(0); // Inicializa como zero
	const [showShareModal, setShowShareModal] = useState(false);

	// Carrega curtidas do localStorage ao montar o componente
	useEffect(() => {
		window.scrollTo(0, 0);
		...
		if (storedLikes[receita.id] !== undefined) {
			...
			setLikes(receita.likes || 0); // Usa o valor padrão se não estiver no localStorage
		}
	}, [receita]);

	// Atualiza receitas recomendadas
	useEffect(() => {
		if (receita) {
	...
	const handleLike = () => {
		const updatedLikes = likes + (likes > receita.likes ? -1 : 1);
		setLikes(updatedLikes);

		// Salva o número de curtidas no localStorage
		const storedLikes = JSON.parse(localStorage.getItem('recipeLikes')) || {};
		storedLikes[receita.id] = updatedLikes;
		localStorage.setItem('recipeLikes', JSON.stringify(storedLikes));
	};
