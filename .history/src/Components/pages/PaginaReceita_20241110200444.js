...
const PaginaReceita = () => {
	...
	const handleLike = () => {
		...
		// Recupera a lista de curtidas do localStorage
		let likedRecipes = JSON.parse(localStorage.getItem('likedRecipes')) || [];
		if (!liked) {
			// Adiciona o ID da receita à lista de curtidas e atualiza o estado
			likedRecipes.push(receita.id);
			setLikes(prevLikes => prevLikes + 1);
			setLiked(true);
		} else {
			// Remove o ID da receita da lista de curtidas e atualiza o estado
			likedRecipes = likedRecipes.filter(recipeId => recipeId !== receita.id);
			setLikes(prevLikes => prevLikes - 1);
			setLiked(false);
		}
		// Salva a lista de curtidas atualizada no localStorage
		localStorage.setItem('likedRecipes', JSON.stringify(likedRecipes));
	};

	// useEffect para restaurar o estado de curtida ao carregar a página
	useEffect(() => {
		const likedRecipes = JSON.parse(localStorage.getItem('likedRecipes')) || [];
		if (likedRecipes.includes(receita.id)) {
			setLiked(true);
		}
	}, [receita]);
