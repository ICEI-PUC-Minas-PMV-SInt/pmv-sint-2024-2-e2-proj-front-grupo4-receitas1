...
const PaginaReceita = () => {
	...
	// Carrega curtidas e estado de "Curtir" do localStorage
	useEffect(() => {
		window.scrollTo(0, 0);
		...
		if (storedLikes[receita.id] !== undefined) {
			setLikes(storedLikes[receita.id].count || receita.likes);
			setIsLiked(storedLikes[receita.id].liked || false);
		} else {
	...
	const handleLike = () => {
		...
		// Salva curtidas e estado de "Curtir" no localStorage
		const storedLikes = JSON.parse(localStorage.getItem('recipeLikes')) || {};
		storedLikes[receita.id] = { count: updatedLikes, liked: !isLiked };
