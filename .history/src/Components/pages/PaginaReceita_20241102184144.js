...
import Comentarios from '../Comentarios.js'; // Importe o componente de comentários

const PaginaReceita = () => {
	const { id } = useParams();
	const receita = receitas.find(r => r.id === Number(id));
	const [receitasRecomendadas, setReceitasRecomendadas] = useState([]);
	const [comentariosVisiveis, setComentariosVisiveis] = useState(false); // Estado para controle dos comentários
				...
				<div className={styles.receitaConteudo}>
					<div className={styles.receitaImagemBotoes}>
						...
						<div className={styles.botoes}>
							...
							{/* Controla o bloco de comentários */}
							<SaveIcon />
							<ShareIcon />
						</div>
					</div>
