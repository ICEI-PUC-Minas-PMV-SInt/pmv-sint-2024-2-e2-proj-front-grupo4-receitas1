...
const PerfilUsuario = () => {
	...
	// Separar preferências selecionadas por categorias
	const selectedPreferencesByCategory = Object.entries(categories).reduce(
		(acc, [category, preferencesList]) => {
			const selected = preferencesList.filter(pref => preferences[pref]);
			if (selected.length > 0) acc[category] = selected;
			return acc;
		},
		{}
	);

	// Função para preparar as preferências selecionadas para o ListaReceitas
	const preferenciasSelecionadas =
		Object.keys(selectedPreferencesByCategory).length > 0
			? selectedPreferencesByCategory
			: {}; // Passa vazio se nenhuma preferência for selecionada
