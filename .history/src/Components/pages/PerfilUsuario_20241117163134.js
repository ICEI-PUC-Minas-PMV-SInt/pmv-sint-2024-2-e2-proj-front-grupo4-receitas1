...
import iconeAddReceita from '../../img/adicionar-receita-icon.webp';
...
const PerfilUsuario = () => {
	...
	const categories = {
		Categorias: ['café da Manhã', 'Almoço', 'Jantar', 'Sobremesa', 'Lanche'],
		'Tipos de Refeição': [
			'macarrão',
			'arroz',
			'legumes',
	...
	// Busca receitas salvas e aplica os filtros
	useEffect(() => {
		...
		const receitasSalvas = todasReceitas.filter(receita =>
		...
		setReceitasFiltradas(
			preferenciasSelecionadas && Object.keys(preferenciasSelecionadas).length
				? receitasSalvas.filter(filtrarReceitas)
				: receitasSalvas
		);
		...
		<>
			<div className={styles.containerBemVindo}>
				...
				>
					...
					<div className={styles.containerAdicionarReceita}>
						<img src={iconeAddReceita}></img>
						<button
