...
const PerfilUsuario = () => {
	...
	const categories = {
		...
		'Estilos Alimentares': ['vegana', 'caldo', 'fitness', 'lowCarb', 'rápido'],
		Tags: ['saudável', 'fácil', 'econômica', 'doces', 'sopas'],
	};
	...
	const preferenciasSelecionadas = Object.entries(categories).reduce(
		(acc, [categoria, preferenciasLista]) => {
			...
			if (selecionadas.length > 0) {
				// Converte as preferências usando o mapeamento para busca
				acc[categoria] = selecionadas.map(pref => mapeamentos[pref] || pref);
			}
			return acc;
		},
		{}
