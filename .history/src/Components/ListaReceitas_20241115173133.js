...
const ListaReceitas = ({ preferenciasSelecionadas = {} }) => {
	// Mapeamentos para nomes alternativos
	const mapeamentos = {
		Café: 'Café da Manhã',
		Caldo: 'Sopa',
	};
			...
			([categoria, preferencias]) => {
				return preferencias.some(preferencia => {
					...
					if (categoria === 'Tipos de Refeição') {
						return (
							receita.tipoRefeicao?.toLowerCase() ===
							preferenciaNormalizada.toLowerCase()
						);
					}
					// Verifica se a palavra aparece no nome da receita ou em ingredientes
					...
					// Verifica propriedades booleanas ou tags
					if (categoria === 'Estilos Alimentares' || categoria === 'Tags') {
						return receita.tags?.includes(preferenciaLower);
					}
